...
import { createExpenditure } from "@/lib/api/expenditure";
import { toast } from "sonner";

export default function ExpendituresAdd({ id }: { id: string }) {
    ...
    const { mutate, isPending } = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success('Berhasil menambahkan pengeluaran', { position: "top-center" })
            setOpen(false);
        },
        onError: (err) => {
            console.error(err);
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button className="cursor-pointer bg-blue-500 hover:bg-blue-600 transition">
                    <Plus size={16} />
                    Pengeluaran
                </Button>
            </DialogTrigger>
